#!/usr/bin/env node
/**
 * ARCH-01J.2: ProductJsonLd old buildProductSeoUrl vs canonicalUrl consumer.
 *
 * Usage: node scripts/verify-product-jsonld-canonical-migration.js
 */
...
function resolveProductJsonLdUrlSource(data) {
...
async function main() {
  ...
  for (const p of products) {
    ...
    productsChecked++;
    const oldUrl = legacyProductJsonLdUrl(data);
    const newUrl = resolveProductJsonLdUrl(data);
    const source = resolveProductJsonLdUrlSource(data);
