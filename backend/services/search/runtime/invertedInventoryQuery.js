/**
 * SEARCH-INVERTED-INDEX-RUNTIME-01 — inverted grouped inventory + preview (index docs only).
 */
...
import { isQualityGateV2Grouping } from "../../../config/searchGroupingConfig.js";
import { pool } from "../../../config/db.js";
import {
  applyGroupQualityGate,
  buildInvertedGroupQualityContext,
} from "./searchGroupQualityGate.js";
 ...
 */
export async function fetchInvertedSharedGroupedInventory(rawQuery, keyword) {
  const exec = await resolveInvertedSearchExecution({ ...rawQuery, keyword, query: keyword });
  exec.queryKeyword = keyword;
  let { vehicleGroups, categoryGroups } = await fetchIndexGroupedInventory(exec, {
    stableKeys: true,
  });

  if (isQualityGateV2Grouping()) {
    const gateCtx = await buildInvertedGroupQualityContext(exec, keyword, rawQuery, pool);
    vehicleGroups = applyGroupQualityGate(vehicleGroups, gateCtx, { grain: "vehicle" });
    categoryGroups = applyGroupQualityGate(categoryGroups, gateCtx, { grain: "category" });
  }
 ...
 */
export async function buildInvertedPreviewBlocks(exec, rankedVehicleGroups, listing, options = {}) {
  ...
  const hydrated = await SearchIndexDocumentReader.readForPreview(
    indexRows.map((r) => ({
