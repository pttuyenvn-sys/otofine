...
import {
  aggregateCategoryVehicleYearRangeLocations,
  isValidSeoYear,
...
function buildCategoryBrandVehicleYearRangeLocationSlug(
  categorySlug,
  brand,
  model,
  yearFrom,
  ...
  return `${categoryCore}-${brandSlug}-${modelSlug}-${yearFrom}-${yearTo}-tai-${locationSlug}`;
}

/**
 * CATEGORY_BRAND_VEHICLE_YEAR_RANGE_LOCATION sitemap inventory from exact fitment pairs.
 *
 * @returns {Promise<{ category: string, categorySlug: string, brand: string, brandSlug: string, model: string, modelSlug: string, location: string, locationSlug: string, yearFrom: number, yearTo: number, productCount: number, slug: string }[]>}
 */
export async function getCategoryBrandVehicleYearRangeLocationSitemapListings() {
  ...
  return aggregateCategoryVehicleYearRangeLocations(rows, { minProductCount: 0 })
    .map((entry) => {
      ...
      const slug = buildCategoryBrandVehicleYearRangeLocationSlug(
        entry.canonicalSlug,
        entry.brand,
        entry.model,
        entry.yearFrom,
