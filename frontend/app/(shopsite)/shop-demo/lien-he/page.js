...
export const metadata = {
  title: `Liên hệ — ${shopDemo.name} | Otofine`,
};

export default function ShopDemoContactPage() {
  return (
        ...
        <ShopSection title="Liên hệ với chúng tôi">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <ContactBlock
              icon={<PhoneIcon />}
              label="Điện thoại"
              value={shopDemo.phone}
              href={`tel:${shopDemo.phone.replace(/\s/g, "")}`}
            />
            <ContactBlock
              icon={<ChatIcon />}
              label="Zalo"
              value={shopDemo.zalo}
              href={`https://zalo.me/${shopDemo.zalo.replace(/\s/g, "")}`}
              external
            />
            <ContactBlock
              icon={<FacebookIcon />}
              label="Facebook"
              value={shopDemo.facebook.label}
              href={shopDemo.facebook.url}
              external
            />
            <ContactBlock
              icon={<MailIcon />}
              label="Email"
              value={shopDemo.email}
              href={`mailto:${shopDemo.email}`}
            />
            <ContactBlock
              icon={<PinIcon />}
              label="Địa chỉ"
              value={shopDemo.address}
              href={`https://www.google.com/maps?q=${encodeURIComponent(shopDemo.address)}`}
              external
              wide
            />
            <ContactBlock
              icon={<ClockIcon />}
              label="Giờ làm việc"
              value={shopDemo.workingHoursLines.join(" · ")}
              wide
            />
          </div>
        </ShopSection>
        ...
        <ShopSection title="Bản đồ" bodyClassName="!p-0">
          <div className="relative aspect-[16/9] rounded-b-2xl overflow-hidden bg-gradient-to-br from-gray-100 via-gray-50 to-white">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center gap-2 text-gray-600">
              <PinIcon large />
              <span className="text-sm">{shopDemo.address}</span>
              <a
                href={`https://www.google.com/maps?q=${encodeURIComponent(shopDemo.address)}`}
                target="_blank"
      ...
      <aside className="lg:col-span-5">
        <ShopSection title="Giờ làm việc chi tiết">
          <ul className="space-y-2 text-sm text-gray-800">
            <Row label="Thứ 2 - Thứ 7" value="08:00 - 18:00" />
            <Row label="Chủ nhật" value="08:00 - 12:00" />
            <Row label="Ngày lễ" value="Theo thông báo" muted />
          </ul>
          <p className="mt-4 text-xs text-gray-500 leading-relaxed">
            Ngoài giờ hành chính, vui lòng liên hệ qua Zalo hoặc Facebook
            để được hỗ trợ sớm nhất. Đơn đặt hàng online được xác nhận
        ...
        <div className="mt-3">
          <ShopSection title="Hỗ trợ nhanh">
            <div className="flex flex-col gap-2">
              ...
              >
                <PhoneIcon /> Gọi ngay: {shopDemo.phone}
              </a>
              <a
                href={`https://zalo.me/${shopDemo.zalo.replace(/\s/g, "")}`}
                target="_blank"
...
function Row({ label, value, muted }) {
  return (
    <li className={`flex items-center justify-between border-b border-gray-100 pb-2 last:border-b-0 last:pb-0 ${muted ? "text-gray-500" : ""}`}>
      <span>{label}</span>
